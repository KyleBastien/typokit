// Competitor benchmark app — standalone Koa.
// Uses Koa with @koa/router, @koa/bodyparser, and better-sqlite3.
...
/** Start the standalone Koa benchmark app */
export async function start(dbPath?: string): Promise<BenchmarkHandle> {
  ...
  router.post("/validate", (ctx) => {
    ...
    if (errors.length > 0) {
      ctx.status = 400;
      ctx.body = { error: 400, message: "Validation failed", fields: errors };
    } else {
      ctx.body = body;
    }
  });
  ...
  // GET /db/:id
  router.get("/db/:id", (ctx) => {
    const id = Number(ctx.params.id);
    if (Number.isNaN(id)) {
      ctx.status = 400;
      ctx.body = { error: 400, message: "Invalid ID" };
      return;
    }
    const row = selectById.get(id) as Record<string, unknown> | undefined;
    if (!row) {
      ctx.status = 404;
      ctx.body = { error: "Not Found", message: `Item ${id} not found` };
      return;
    }
  ...
  // GET /middleware — 5 no-op middleware layers then response
  router.get(
    "/middleware",
    noopMiddleware,
    noopMiddleware,
