...
1. You define types once in `@app/schema` using JSDoc annotations
...
<Aside type="tip">
Because TypoKit only generates types and schemas, you can switch database tools (e.g., Drizzle → Kysely) by swapping the adapter and re-running the build. Your `@app/schema` types don't change.
</Aside>
...
Every adapter receives the same input — a `SchemaTypeMap` built from your `@app/schema` types. JSDoc tags on your types drive the database schema generation:
...
|---------|-------|---------|
| Type definitions | **TypoKit** | `@app/schema` with JSDoc annotations |
