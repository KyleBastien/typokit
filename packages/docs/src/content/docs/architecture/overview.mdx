...
<Aside type="tip">
While the default pipeline generates TypeScript artifacts compiled by `tsc`, plugins can target other languages entirely. The `@typokit/plugin-axum` plugin generates a complete Rust/Axum server from the same TypeScript schema types, using the `compile` hook to run `cargo build` instead of `tsc`. See the [Building a Rust/Axum Server](/typokit/guides/rust-axum-server/) guide for details.
</Aside>
