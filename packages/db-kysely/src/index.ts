...
function generateFile(
  ...
  const tableInterfaces: Array<{ tableName: string; interfaceName: string; code: string }> = [];
  let needsGenerated = false;
  ...
  for (const [typeName, meta] of Object.entries(types)) {
    ...
    tableInterfaces.push({ tableName, interfaceName: result.interfaceName, code: result.code });
  }
...
function generateMigrationSql(changes: SchemaChange[], dialect: KyselyDialect): string {
  const lines: string[] = [];
